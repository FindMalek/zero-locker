...
export function useAggressiveFormBlocker() {
  useEffect(() => {
    const blockFormSubmission = (e: Event) => {
      ...
      if (target && (
        target.hasAttribute?.('data-testid') && target.getAttribute('data-testid')?.includes('vault-form') ||
    ...
    const blockEnterKey = (e: KeyboardEvent) => {
      if (e.key === 'Enter') {
        ...
        if (target && (
          target.hasAttribute?.('data-form-type') && target.getAttribute('data-form-type') === 'other' ||
          target.hasAttribute?.('data-form-type') && target.getAttribute('data-form-type') === 'isolated' ||
          target.closest?.('[data-testid*="vault-form"]') ||
          target.closest?.('[data-form-isolated]')
        )) {
          e.preventDefault()
