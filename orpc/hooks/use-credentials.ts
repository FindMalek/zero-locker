...
export function useCredential(id: string) {
  return useQuery(orpc.credentials.get.queryOptions({
    input: { id },
    enabled: !!id,
  }))
}
...
export function useCredentials(input: ListCredentialsInput = { page: 1, limit: 10 }) {
...
export function useCreateCredential() {
  ...
  return useMutation(orpc.credentials.create.mutationOptions({
...
export function useUpdateCredential() {
  ...
  return useMutation(orpc.credentials.update.mutationOptions({
    ...
    onSuccess: (updatedCredential) => {
      ...
      queryClient.setQueryData(
        orpc.credentials.get.queryKey({ input: { id: updatedCredential.id } }),
        updatedCredential
      )
...
export function useDeleteCredential() {
  ...
  return useMutation(orpc.credentials.delete.mutationOptions({
    onMutate: async (input) => {
      // Cancel any outgoing refetches
      await queryClient.cancelQueries({
        queryKey: orpc.credentials.get.key({ input: { id: input.id } })
