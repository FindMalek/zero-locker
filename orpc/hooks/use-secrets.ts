...
export function useSecret(id: string) {
  return useQuery(orpc.secrets.get.queryOptions({
    input: { id },
    enabled: !!id,
  }))
}
...
export function useSecrets(input: ListSecretsInput = { page: 1, limit: 10 }) {
  return useQuery(orpc.secrets.list.queryOptions({
    input,
...
export function useCreateSecret() {
  ...
  return useMutation(orpc.secrets.create.mutationOptions({
    onSuccess: (newSecret) => {
...
export function useUpdateSecret() {
  ...
  return useMutation(orpc.secrets.update.mutationOptions({
...
export function useDeleteSecret() {
  ...
  return useMutation(orpc.secrets.delete.mutationOptions({
    onMutate: async (input) => {
      // Cancel any outgoing refetches
      await queryClient.cancelQueries({
        queryKey: orpc.secrets.get.key({ input: { id: input.id } })
